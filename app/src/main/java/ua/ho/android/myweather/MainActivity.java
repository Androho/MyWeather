...
//        String lang=String.valueOf(current_location.getLatitude());
        ...
        Intent getLtLg = getIntent();

        latlangGeo = getLtLg.getExtras().getString("location");
        //"50.4501,30.5234";
...
//                                    + hourly.getHourlyForecast().get(0).getIcon() + ".svg";
                        icName = hourly.getHourlyForecast().get(0).getIcon();

                        resId = MainActivity.this.getResources().getIdentifier(icName, "drawable", MainActivity.this.getPackageName());

                        Picasso.with(MainActivity.this)
                                .load(R.drawable.clear)
                                .error(resId)
                                .into(mWeatherIcon);
                        DataPoint[] mDataPoints = new DataPoint[hourly.getHourlyForecast().size()];
                        String[] dataTime = new String[hourly.getHourlyForecast().size()];
                        for (int i = 0; i < hourly.getHourlyForecast().size(); i++) {
                            String time = hourly.getHourlyForecast().get(i).getFCTTIME().getWeekdayName() + " " + hourly.getHourlyForecast().get(i).getFCTTIME().getHour() + ":00";
                            int temp = Integer.parseInt(hourly.getHourlyForecast().get(i).getTemp().getMetric());
                            mDataPoints[i] = new DataPoint(i, temp);
                            dataTime[i] = new String(time);
                        }
                        mSeries = new LineGraphSeries<>(mDataPoints);
                        mGraph.getViewport().setScalable(true);
                        mGraph.getViewport().setScrollable(true);
                        StaticLabelsFormatter staticLabelsFormatter = new StaticLabelsFormatter(mGraph);
                        staticLabelsFormatter.setHorizontalLabels(dataTime);
